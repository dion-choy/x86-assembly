...
; Compile with NASM:
; nasm.exe -f bin .\qrcode.asm -o qrcode.com
...
_start:
    ...
    validNum:
    cmp byte [maskNum], 0FFh   ; check if maskNum written to
    jne getMask         ; if yes then ignore input

    mov byte [maskNum], al     ; if no then write to maskNum
    ...
    addPadding:
    inc bx
    mov byte strIn[bx], 0ECh

    inc bx
    mov byte strIn[bx], 011h
    ...
    mov bl, str1[bx]         ; change D word to exponent
    ...
    pop bx
    xor str1[bx-1+di], al   ; xor with D word
...
drawPx: ; dh input row, dl input col
    push ax
    push cx
    push dx
...
getPx: ; dh input row, dl input col
    push cx
    push dx
